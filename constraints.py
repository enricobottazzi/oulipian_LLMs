...
class LipogramConstraint(LogitsProcessor):
    "Forbid tokens containing the banned letter (case insensitive). Stateless."

    def __init__(self, banned_letter: str, tokenizer):
        assert banned_letter.lower() in string.ascii_lowercase, "banned_letter must be a-z"
        self.banned_token_ids = _ban(tokenizer, {banned_letter.lower()})
...
class UnivocalConstraint(LogitsProcessor):
    "Forbid tokens containing any vowel other than the allowed one (case insensitive). Stateless."
